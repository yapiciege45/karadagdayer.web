...
export const CustomerHomeContainer = () => {
  ...
  const saveChanges = async() => {
    ...
    if(proxyImg) {
      const proxyRes = await fetch(`${process.env.API_URL}/customer/add/proxy`, {
        method: 'POST',
        headers: {
    ...
    if(diplomaImg) {
      const diplomaRes = await fetch(`${process.env.API_URL}/customer/add/diploma`, {
        method: 'POST',
        headers: {
          'Content-Type': 'image/jpeg',
          'Authorization': `Bearer ${token}`
    ...
    if(passportImg) {
      const diplomaRes = await fetch(`${process.env.API_URL}/customer/add/passport`, {
        method: 'POST',
        headers: {
          'Content-Type': 'image/jpeg',
          'Authorization': `Bearer ${token}`
    ...
    if(criminalRecordImg) {
      const criminalRecordRes = await fetch(`${process.env.API_URL}/customer/add/criminal-record`, {
        method: 'POST',
        headers: {
          'Content-Type': 'image/jpeg',
          'Authorization': `Bearer ${token}`
